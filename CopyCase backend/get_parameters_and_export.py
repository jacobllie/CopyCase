...
def get_parameters_and_export(initials, destination, patient, case, export_files=True):
    ...
    for plan in case.TreatmentPlans:
        ...
        # Skipping approved plans
        if plan.Review.ApprovalStatus == "Approved":
            print("Plan {} er Approved og kan ikke eksporteres med scriptable export og må eksporteres manuelt.".format(plan.Name))
            continue

        # TODO: Fiks en mer robust måte å sjekke etter importerte planer, f.eks. hvis dose er considered clinical.
        #  Det funker ikke fordi alle doser isClinical for planer som har dose og ikke er importert. Hva er det som er unikt med
        #  Man må også ta høyde for planer som er importert men som er unapproved. Finnes det en indikator på at planen er importert.
        #  Imported plan ser ut til å
        #  Skip approved planer
        try:
